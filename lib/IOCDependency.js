...
class IOCDependency {
  ...
  constructor(src, parameters, ioc) {
    // If it is a string...
    ...
    // In the case where the parameters contain another service, convert the sub-service to a function
    if (this.parameters) {
      Object.keys(this.parameters).forEach((key) => {
        if (this.isDependency(this.parameters[key]))
          // Replace key with a function that injects the service from the passed-in IOC
          this.parameters[key] = () => ioc.inject(key);
      });
    }
  }
  ...
  isDependency() {
    return (
      (typeof item === 'function' && item.prototype instanceof IOCService) ||
      (typeof item === 'object' && item['src'].prototype instanceof IOCService)
    );
  }
